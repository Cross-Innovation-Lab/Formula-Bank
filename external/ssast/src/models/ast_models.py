...
class ASTModel(nn.Module):
    def __init__(self, label_dim=527,
        ...
        # The original SSAST release pinned timm 0.4.5.  The project uses the
        # same DeiT module API with timm 1.x; keep the version information
        # visible without preventing a checkpoint-compatible fine-tuning run.
        if timm.__version__ != '0.4.5':
            warnings.warn(
                f"SSAST was released with timm 0.4.5; running the compatible "
                f"DeiT path with timm {timm.__version__}.",
                RuntimeWarning,
            )
        ...
        if pretrain_stage == True:
            ...
            # timm >= 1.0 keeps a strict 384x384 image-size assertion in its
            # PatchEmbed implementation.  SSAST replaces the projection and
            # accepts variable log-Mel sizes below, so retain that intended
            # behavior on the newer timm implementation as well.
